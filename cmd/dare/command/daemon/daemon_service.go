...
import (
	...
	decryptor "github.com/da-moon/coe817-dare/cmd/decryptor-plugin/handler"
	...
	encryptor "github.com/da-moon/coe817-dare/cmd/encryptor-plugin/handler"
...
func (s *Service) Encrypt(r *http.Request, req *model.EncryptRequest, res *model.EncryptResponse) error {
	...
	resTmp, err := (&encryptor.Encrypt{}).Encrypt(req)
	if err != nil {
		err = stacktrace.Propagate(err, "encryptor failed to encrypt given input")
		s.logger.Printf("[DEBUG] error : %#v", err)
		return err
	}
	res.SourceHash = resTmp.SourceHash
	res.DestinationHash = resTmp.DestinationHash
...
func (s *Service) Decrypt(r *http.Request, req *model.DecryptRequest, res *model.DecryptResponse) error {
	...
	resTmp, err := (&decryptor.Decrypt{}).Decrypt(req)
