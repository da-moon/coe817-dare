...
// ## Simple Subcommand
//
// every subcommand is defined in a file that shares the same name as the subcommand
...
// - Help : detailed instructions of what the subcommand does. make sure to change
...
// through flags and environment variables, why ? to simplify operation for the end user
...
// I put all flags in a centralized package to make modification and code review simpler because
...
// was passed in through env variables or flags, I would set the values to a default value ( if we can set a default value )
//
// In flags package, there is helper methods in `flag_slice_value.go` I have else defined a function
