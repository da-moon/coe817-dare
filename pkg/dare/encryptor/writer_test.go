...
func TestWriter(t *testing.T) {
	...
	encryptor := encryptor.NewWriter(
		writer,
		nonce,
		sharedKey,
	)
	_, err := encryptor.Write(testString)
	if err != nil {
		t.Error(err)
	}
	writer.Flush()
	if !bytes.Equal(expectedEncryptionResult, resultBuffer.Bytes()) {
		t.Errorf("want: %v \n got: %v", expectedEncryptionResult, resultBuffer.Bytes())
	}
}
